...
class RecommenderTrainer:
    ...
    def _update_actor(self, batch: List[Dict]) -> float:
        ...
        # print(f"States shape: {states.shape}")  # Debería ser [batch_size, HIDDEN_DIM + 32]
        
        actions = self.actor.forward_from_state(states)
        # print(f"Actions shape: {actions.shape}")  # Debería ser [batch_size, EMBEDDING_DIM]
        
        q_values = self.critic(states, actions)
        # print(f"Q values shape: {q_values.shape}")  # Debería ser [batch_size, 1] o [batch_size]
    ...
    def train_epoch(self, epsilon: float = 0.1, max_steps: int = 1000, print_logs: bool = False) -> Dict[str, float]:
        ...
        for i, item in enumerate(train_items):
            ...
            if np.random.random() < epsilon:
                random_action = torch.randn_like(action_vec)
                action_vec = random_action
        ...
        metrics = self._compute_metrics(user_history, train_items, SplitType.TRAIN)
    ...
    def evaluate(self, split_type: SplitType = SplitType.VALIDATION, 
        ...
        metrics = self._compute_metrics(user_history, eval_items, split_type)
    ...
    def _compute_metrics(self, recommendations: List[Dict], 
        ...
        """Calcula métricas de evaluación corregidas"""
        ...
        # 1. Recompensas (limitar a 100 para consistencia)
        max_items = min(100, len(recommendations), len(ground_truth))
        rec_rewards = [r.get('reward', 0) for r in recommendations[:max_items]]
        gt_rewards = [r.get('reward', 0) for r in ground_truth[:max_items]]
        ...
        # 2. IDs para precision/recall
        rec_track_ids = [r.get('track_id') for r in recommendations[:max_items] if 'track_id' in r]
        gt_track_ids = [r.get('track_id') for r in ground_truth[:max_items] if 'track_id' in r]
        
        # Convertir a sets para cálculos de intersección
        rec_set = set(rec_track_ids)
        gt_set = set(gt_track_ids)
        
        # 3. Precision, Recall y MAP@k
        for k in k_values:
            if len(rec_track_ids) >= k and len(gt_track_ids) > 0:
                # Precision@k y Recall@k
                k_rec = rec_track_ids[:k]
                k_gt = gt_track_ids[:k]
                
                # Intersección relevante (usamos los primeros k de ground truth como relevantes)
                relevant_in_k = len(set(k_rec) & set(k_gt))
                
                precision = relevant_in_k / k
                recall = relevant_in_k / min(k, len(set(gt_track_ids)))
                ...
                if k >= 2:  # MAP solo tiene sentido para k > 1
                    map_score = self._calculate_map_at_k(rec_track_ids, gt_track_ids, k)
        ...
        if rec_rewards and gt_rewards:
            for k in k_values:
                ...
                if k_valid > 0:
                    # Obtener recompensas de las recomendaciones (ordenadas como aparecen)
                    rec_k_rewards = rec_rewards[:k_valid]
                    
                    # Calcular DCG: sum(relevance_i / log2(i+2))
                    dcg = 0.0
                    for i, reward in enumerate(rec_k_rewards):
                        dcg += reward / math.log2(i + 2)  # i+2 porque i empieza en 0
                    
                    # Calcular IDCG: ordenar ground truth de mayor a menor
                    # Usamos las recompensas de ground_truth, no las de recommendations
                    ideal_rewards = sorted(gt_rewards[:k_valid], reverse=True)
                    idcg = 0.0
        ...
        context_coverage = self._compute_context_coverage(recommendations[:max_items])
        metrics.update(context_coverage)
        
        # 6. Métricas adicionales de diversidad
        diversity_metrics = self._compute_diversity_metrics(recommendations[:max_items])
